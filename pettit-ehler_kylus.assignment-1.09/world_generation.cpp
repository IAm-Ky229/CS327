...
int main(int argc, char *argv[]) {
  ...
  // Store information about the player character (pokemon and items)
  // Seperate from the PC position struct so we don't have to keep reallocating this memory
  PC_state player_character;
...
void mapGen::select_starting_pokemon(std::vector<in_game_pokemon> starting_pkmn, PC_state &PC) {
  ...
  mvaddstr(1, 20, "CHOOSE A LV 1 POKEMON");

  sprintf(buffer1, "%s", starting_pkmn[0].get_name().c_str());
