...
void place_border_boulders() {
  ...
  for(i = 0; i < HORIZONTAL; i++) {
    generate_map[i][0] = boulder;
  }
  
  for(i = 0; i < HORIZONTAL; i++) {
  generate_map[i][VERTICAL - 1] = boulder;
  }
  
  for(i = 0; i < VERTICAL; i++) {
     generate_map[0][i] = boulder;
  }
 
  for(i = 0; i < VERTICAL; i++) {
   generate_map[HORIZONTAL - 1][i] = boulder;
  }
  ...
  while(number_clearings < 2) {
	...
	for(k = x_pos - length; k < x_pos + height; k++) {
	  if((j > 0) && (j < 21) && (k > 0) && (k < 80)
	     && generate_map[k+1][j] != clearing) {
	    generate_map[k][j] = clearing;
	  }
	}
      }
    }
...
void place_tall_grass() {
  ...
  while(number_tall_grasses < 2) {
    ...
    for(j = y_pos - height; j < y_pos + height; j++) {
      for(k = x_pos - length; k < x_pos + height; k++) {
	if(generate_map[k][j] == tall_grass) {
	  overlap = 1;
	}
      }
    }
	...
	for(k = x_pos - length; k < x_pos + height; k++) {
	  if((j > 0) && (j < 21) && (k > 0) && (k < 80)
	     && generate_map[k+1][j] != tall_grass) {
	    generate_map[k][j] = tall_grass;
	  }
	}
      }
    }
...
void place_buildings() {
  ...
  while(selected_row == exit_right
	|| selected_row == exit_left) {
    selected_row = (rand() % (18 - 3 + 1)) + 3;
  }

  // We don't want to place a building on the literal exit row/column
  // We also don't want the mart and the center to overlap
  while(selected_column == exit_top
	|| selected_column == exit_bottom
...
void place_pokemart(int selected_row) {
  ...
  // Iterate over entire row
    while(j < HORIZONTAL) {
      ...
      // If we find a path
      if(generate_map[j][selected_row] == path) {
	...
	generate_map[j-2][selected_row] = pokemon_mart;
	generate_map[j-1][selected_row] = pokemon_mart;
	generate_map[j-1][selected_row - 1] = pokemon_mart;
	generate_map[j-2][selected_row - 1] = pokemon_mart;
	break;
      }
      j++;
    }
...
void place_pokecenter(int selected_column) {
  ...
  int j = 0;
    ...
    while(j < VERTICAL) {
      if(generate_map[selected_column][j] == path) {
	
	// We don't need to check bounds, as roads cannot be less
	// Than 2 units away from the edges
	// Also, we are going to the first instance of a road, so
	// we can just place the building behind us and avoid checking anything
	generate_map[selected_column][j-2] = pokemon_center;
	generate_map[selected_column][j-1] = pokemon_center;
	generate_map[selected_column - 1][j-1] = pokemon_center;
	generate_map[selected_column - 1][j-2] = pokemon_center;
	break;
	}  
      j++;
    }
...
void place_exits() {
  ...
  exit_right = (rand() % (17 - 3 + 1)) + 3;
  exit_bottom = (rand() % (76 -32 + 1)) + 3;
  exit_left = (rand() % (17 - 3 + 1)) + 3;
  exit_top = (rand() % (76 - 3 + 1)) + 3;

  generate_map[HORIZONTAL - 1][exit_right] = border_exit;
  generate_map[exit_bottom][VERTICAL - 1] = border_exit;
  generate_map[0][exit_left] = border_exit;
  generate_map[exit_top][0] = border_exit;
...
void place_paths() {
  ...
  while (connect_top_bottom_counter < connect_top_bottom) {
    generate_map[exit_top][connect_top_bottom_counter] = path;
  ...
  while(connect_top_bottom_counter > connect_top_bottom - 1) {
    generate_map[exit_bottom][connect_top_bottom_counter] = path;
    connect_top_bottom_counter--;
  }

  // If the top exit is farther right, add a connecting road to the bottom path
  if(exit_top > exit_bottom) {
    connect_paths = exit_bottom;
    while(connect_paths < exit_top + 1) {
      generate_map[connect_paths][connect_top_bottom_counter] = path;
      connect_paths++;
    }
  }
  // Else, add a connecting road to the top path
  else {
      connect_paths = exit_top;
      while(connect_paths < exit_bottom + 1) {
	generate_map[connect_paths][connect_top_bottom_counter] = path;
	connect_paths++;
      }
  }
  ...
  while (connect_right_left_counter < connect_right_left) {
    generate_map[connect_right_left_counter][exit_left] = path;
  ...
  while(connect_right_left_counter > connect_right_left - 1) {
    generate_map[connect_right_left_counter][exit_right] = path;
    connect_right_left_counter--;
  }

  // If the right exit is above the left exit
  if(exit_right < exit_left) {
    connect_paths = exit_right;

    // Build the connecting path down from the left exit
    while(connect_paths < exit_left + 1) {
      generate_map[connect_right_left_counter][connect_paths] = path;
      connect_paths++;
    }
  }
  // Otherwise, build the connecting path down from the right exit
    else {
      connect_paths = exit_left;
      while(connect_paths < exit_right + 1) {
	generate_map[connect_right_left_counter][connect_paths] = path;
	connect_paths++;
      }
    }
